...
スキルと CLI は分離されています — CLI は GitHub CDN からスキルインデックスを取得します。スキルの追加や更新は CLI リリースを**必要としません**。

**ワークフロー：**

```bash
# 1. public/ でスキルを修正または新規追加
# 2. スキルインデックスを再構築
cd cli && npm run build-index

# 3. コミットしてプッシュ — CDN が自動的に更新
git add public/ index.json
git commit -m "feat: add new-skill"
git push
```

次回 CLI を実行したユーザーは新スキルを利用できます。

**新 CLI バージョンをリリースするのは以下のケースのみ：**
- CLI 機能の追加またはバグ修正
- `build-index.ts` のインデックス生成ロジック変更
