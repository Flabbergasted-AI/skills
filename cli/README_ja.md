...
skills install <名前> --lock        # インストールしてskills.lockに書き込む
skills install <名前> --frozen      # skills.lockの固定バージョンでインストール
...
```bash
skills upgrade                      # CLIの最新バージョンを確認・インストール
skills upgrade --check              # 確認のみ、インストールしない
```

### バージョンロック

```bash
skills lock create                  # インストール済みスキルからskills.lockを生成
skills lock show                    # ロックされたバージョンを表示
skills lock update [名前...]        # ロックエントリを最新コミットに更新
skills lock remove <名前...>        # ロックファイルからスキルを削除
```
